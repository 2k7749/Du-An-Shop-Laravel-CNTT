...
<div class="grid grid-cols-12 gap-6 mt-5">
  <div class="intro-y col-span-12 flex flex-wrap sm:flex-no-wrap items-center mt-2">
      <button class="button text-white bg-theme-1 shadow-md mr-2"><a href="{{URL::to('/sc_admin/add-product')}}">Thêm mới sản phẩm</a></button>
      ...
      <table class="table table-report -mt-2">
          <thead>
              <tr>
                  ...
                  <th class="whitespace-no-wrap">Hình sản phẩm</th>
                  <th class="whitespace-no-wrap">Danh mục</th>
                  <th class="whitespace-no-wrap">Thương hiệu</th>
          ...
          <tbody>
            ...
            @if ($all_product!=null)
                ...
                <td><label class="i-checks m-b-none"><input type="checkbox" name="post[]"><i></i></label></td>
                    ...
                    <td><div class="font-medium whitespace-no-wrap">{{number_format($product->product_price).' vnđ'}}</div></td>
                    ...
                    <td class="w-40">
                        ...
                        if($product->product_status==0){?>
                            ...
                            <a href="{{URL::to('/sc_admin/active-product/'.$product->product_id)}}"><i data-feather="check-square" class="w-4 h-4 mr-2"></i>  Không hiển thị</a>  
                            </div>
                        <?php
                        }else{
                        ?>
                            <div class="text-center flex items-center justify-center text-theme-9"> 
                            <a href="{{URL::to('/sc_admin/unactive-product/'.$product->product_id)}}"><i data-feather="check-square" class="w-4 h-4 mr-2"></i>  Hiển thị</a>  
                            </div>
                        <?php
                        }
                        ?>
                    ...
                    <td class="table-report__action w-56">
                        <div class="flex justify-center items-center">
                            <a class="flex items-center mr-3" href="{{URL::to('/sc_admin/edit-product/'.$product->product_id)}}"> <i data-feather="check-square" class="w-4 h-4 mr-1"></i> Sửa </a>
                            <a class="flex items-center text-theme-6" onclick="return confirm('Bạn có muốn xoá sản phẩm này?')" href="{{URL::to('/sc_admin/delete-product/'.$product->product_id)}}" data-toggle="modal" data-target="#delete-confirmation-modal"> <i data-feather="trash-2" class="w-4 h-4 mr-1"></i> Xoá </a>
                        </div>
                    </td>
                </tr>
                @endforeach
